...
remote_file "#{node[:hiway][:software][:dir]}/#{node[:hiway][:variantcall][:varscan][:jar]}" do
...
bash "install_varscan" do
  ...
  code <<-EOF
  set -e && set -o pipefail
  ln -s #{node[:hiway][:software][:dir]}/#{node[:hiway][:variantcall][:varscan][:jar]} \$JAVA_HOME/jre/lib/ext/
  EOF
