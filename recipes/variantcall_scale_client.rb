...
bash "copy_data_to_hdfs" do
  ...
  code <<-EOH
  set -e && set -o pipefail
    #{node[:hadoop][:home]}/bin/hdfs dfs -put #{node[:saasfee][:variantcall][:scale][:data]} #{node[:saasfee][:hiway][:hdfs][:basedir]}/hg19
  EOH
  not_if "#{node[:hadoop][:home]}/bin/hdfs dfs -test -e #{node[:saasfee][:hiway][:hdfs][:basedir]}/hg19"
end
