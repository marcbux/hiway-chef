...
remote_file "#{Chef::Config[:file_cache_path]}/#{node[:saasfee][:variantcall][:bwa][:tarbz2]}" do
  source node[:saasfee][:variantcall][:bwa][:url]
  owner node[:saasfee][:user]
  group node[:hadoop][:group]
  mode "775"
  action :create_if_missing
...
bash "install_bwa" do
  ...
  code <<-EOH
  set -e && set -o pipefail
    tar xjvf #{Chef::Config[:file_cache_path]}/#{node[:saasfee][:variantcall][:bwa][:tarbz2]} -C #{node[:saasfee][:software][:dir]}
    make -C #{node[:saasfee][:variantcall][:bwa][:home]}
  EOH
  not_if { ::File.exist?("#{node[:saasfee][:variantcall][:bwa][:home]}") }
end
...
bash 'update_env_variables' do
  ...
  code <<-EOH
  set -e && set -o pipefail
    ln -s #{node[:saasfee][:variantcall][:bwa][:home]}/bwa /usr/bin/
  EOH
